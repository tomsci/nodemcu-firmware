...
static TString *newlstr (lua_State *L, const char *str, size_t l,
  ...
  if (strtype == LUAS_PREALLOCED_STRING)
    ...
    ts = cast(TString *, luaM_malloc(L, sizeof(TString) + (strtype == LUAS_READONLY_STRING ? sizeof(char**) : (l+1)*sizeof(char))));
  ts->tsv.len = l;
  ts->tsv.hash = h;
...
static TString *luaS_newlstr_helper (lua_State *L, const char *str, size_t l, int strtype) {
  ...
  if (strtype != LUAS_PREALLOCED_STRING) {
    strtype = (lua_is_ptr_in_ro_area(str) && l+1 > sizeof(char**) &&
...
TString *luaS_buftostr (lua_State *L, TString *ts) {
  TString* result = luaS_newlstr_helper(L, cast(const char *, ts + 1), ts->tsv.len, LUAS_PREALLOCED_STRING);
  if (result != ts) {
    /* Must have reused an existing string */
    lua_freebuf(L, ts + 1);
  }
  return result;
}
